...
const TestPage = () => {
  ...
  useEffect(() => {
    ...
    const startQuiz = async () => {
      try {
        // Map difficulty to category for better questions
        const category = difficulty === 'easy' ? 'general' : 
                        difficulty === 'hard' ? 'science' : 
                        difficulty === 'advance' ? 'computer' : 'general';
        
        const difficultyLevel = difficulty === 'easy' ? 'easy' : 
                               difficulty === 'hard' ? 'medium' : 
                               difficulty === 'advance' ? 'hard' : 'medium';
        
        console.log('Starting quiz with:', { category, difficultyLevel });
        const quizData = await quizAPI.startQuiz(category, difficultyLevel);
        ...
        for(let i = 1; i <= quizData.totalQuestions; i++) {
          try {
            ...
            allQuestions.push({
              ...
              correctAnswer: questionData.options[0], // We'll handle this differently
